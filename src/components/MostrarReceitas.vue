...
export default {
    ...
    async created() {
        ...
        this.receitasEncontradas = receitas.filter((receita) => {
            receita.ingredientes.find((ingrediente) =>
                this.ingredientes.includes(ingrediente)
            );

            const receitaPodeSerFeita = itensDeLista1EstaoEmLista2(receita.ingredientes, this.ingredientes);
            return receitaPodeSerFeita;
        })
    },
    props: {
